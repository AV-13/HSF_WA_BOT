/**
 * Histoire Sans Faim WhatsApp Bot
...
// Verify NGROK_URL is loaded
if (process.env.NGROK_URL) {
  console.log(`✅ NGROK_URL loaded: ${process.env.NGROK_URL}`);
} else {
  console.warn('⚠️ NGROK_URL not set in .env file - menus won\'t work with Meta!');
}
...
// Serve static files from assets/menus folder (for PDF menus)
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const menusPath = join(__dirname, '..', 'assets', 'menus');
app.use('/menus', express.static(menusPath));
...
app.get('/', (req: Request, res: Response) => {
  res.json({
    status: 'running',
    service: 'Histoire Sans Faim WhatsApp Bot',
    version: '2.0.0',
...
const server = app.listen(PORT, () => {
  console.log('\n🚀 Histoire Sans Faim WhatsApp Bot is running!');
