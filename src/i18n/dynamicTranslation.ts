...
import { getHistoireSansFaimAgent } from '../agent/mastra.js';
...
export async function generateText(
  ...
  try {
    ...
    const prompt = `You are a professional translator for Histoire Sans Faim, a welcoming Italian restaurant in Lausanne.
...
export async function generatePrompt(
  ...
  const promptDescriptions: Record<string, string> = {
    'choose_menu_prompt': 'Tell the user we offer 2 different menus (Menu du moment and Brunch) and ask which one they want to see',
...
export async function generateErrorMessage(
  ...
  const errorDescriptions: Record<string, string> = {
    'technical': 'Apologize for a technical issue and provide contact information: Phone: +41 21 648 22 22, Email: info@histoiresansfaim.ch',
